...
- 任一输出不得等于、包含或位于任一输入之内；多个输出（正式结果、失败诊断、显式日志）之间也不得形成相同或祖先/后代关系；该检查必须发生在首次写日志之前，ResearchBuilder 的直接调用同样执行；
- 拒绝输出到文件系统根、工作区根覆盖和通过符号链接逃逸；
- 正式产物路径从叶节点到所有已存在父节点均拒绝符号链接和 Windows junction；
...
| CSV 公式注入 | 可配置的人类标签若以 `= + - @` 开头则在配置阶段拒绝；其他 CSV 人类文本在输出边界安全编码 |
